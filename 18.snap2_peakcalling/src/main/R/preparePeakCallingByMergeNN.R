...
# k49 used later, which is k50 with more cells.
# though it captures less cl, but more on subclasses (24 / 25)
# 206 out of 215 of them have the same subclasses

tfLightSum2 <- tfLightSum[,
  c("L4", "k49_cl_tf", "k49_subclass_id", "L4size.x")]
tfLightSum2$L4size <- tfLightSum2$L4size.x
tfLightSum2$L4size.x <- NULL
...
L4Group4PeakCalling <- R6::R6Class(
  ...
  public = list(
    ...
    mergeL4 = function(groupBy = "k49_cl_tf", newGroupName = "cl") {
...
mergeLightSumList <- smallLightSum2 |>
  ...
  x => map(.x = x, .f = \(y) {
    y$mergeL4(groupBy = "k49_cl_tf", newGroupName = "cl")
  }) |>
...
write.table(
  x = L4pc2sizes,
  file = file.path(projdir, "18.snap2_peakcalling",
    "src/main/resource", str_glue("{group}_L4pc2sizes_{method}-k49-cl_v1.csv")),
  quote = FALSE,
  row.names = FALSE,
  col.names = FALSE,
  sep = ","
