...
parser$add_argument("-i", "--input",
  required = TRUE, help = "list of summit bed")
parser$add_argument("-g", "--genome",
  default = "mm10", help = "used genome [default %(default)s]")
parser$add_argument("-e", "--extend",
  default = 500, help = "extend to [default %(default)s] bp length")
parser$add_argument("--blacklist",
  default = "/projects/ps-renlab/yangli/genome/mm10/mm10.blacklist.bed.gz",
  help = "blacklist in bed gz")
parser$add_argument("--chromSize",
  default = "/projects/ps-renlab/yangli/genome/mm10/mm10.chrom.sizes",
  help = "chrom size")
parser$add_argument("-d", "--dir", required = TRUE,
  help = "output dir")
parser$add_argument("-o", "--output", required = TRUE,
  help = "output file prefix")
parser$add_argument("--ncpu", default = 4)
parser$add_argument("--dsum", required = TRUE)
...
if (!dir.exists(outsumDir)) {
  message("Create outsumDir: ", outsumDir)
  dir.create(outsumDir, recurisive = TRUE)
}
...
outfname <- file.path(outsumDir,
  paste0(outF, ".filteredNfixed.union.peakSet"))
message("save union peak set to: ", outfname)
fwrite(outUnion, file = outfname,
  sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
message("Done.")
