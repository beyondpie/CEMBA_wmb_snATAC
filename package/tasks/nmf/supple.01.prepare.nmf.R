...
# * configs
outdir <- "data"
outh5 <- file.path(outdir, "cpm.cbyp.novlp.Intv2.h5")
outPeakCoordFile <- file.path(outdir, "peaks.novlp.Intv2.txt")
outClusterFile <- file.path(outdir, "clusters.novlp.Intv2.txt")

# * load atac cpm.
cpm.scbyp <- readRDS(cembav2env$subclassPmatCPMIntv2File)

## # * load cCREs from posivite pdc
novlp.CREs <- data.table::fread(
  file = cembav2env$nonOvlpDHSPeakBedFile,
  sep = "\t",
  header = FALSE,
  data.table = FALSE
)
colnames(novlp.CREs) <- c("chr", "start", "end", "name")
novlpCoords <- with(novlp.CREs,
  paste(chr, paste(start, end, sep = "-"), sep = ":"))

cpm.scbyp <- cpm.scbyp[ , novlpCoords]
