...
#' @export
getIndexOfUniqueName <- function(names) {
  # by ChatGPT-4
  uniqUpToDown <- !duplicated(names)
  uniqDownToUp <- !duplicated(names, fromLast = TRUE)
  return(which(uniqUpToDown & uniqDownToUp))
}

#' logging with standard out and error to log file
#' generated by ChatGPT4
#' if used in REPL, remember to call
#' sink(NULL, type = "output")
#' sink(NULL, type = "message")
#' stop redicting output to log file
#' @depends logger
#' @export
setupLogging <- function(log_file_path, threshold = logger::TRACE) {
  con <- file(log_file_path, open = "wt")
...
#' @export
getTopkColScoreWithName <- function(mat,
                                    topk = 3,
                                    decreasing = TRUE) {
  rnames <- rownames(mat)
  k <- min(topk, nrow(mat))
  lapply(seq_len(ncol(mat)), \(i) {
    vals <- mat[, i]
    rank <- order(vals, decreasing = decreasing)
    names <- rnames[rank][1:k]
    scores <- vals[rank][1:k] |>
      setNames(object = _, nm = names)
    return(scores)
  }) |>
    setNames(object = _, nm = colnames(mat))
}
